...
class Particle:
    ...
    def fitness(self):
        ...
        for i in range(len(X)):
            ...
            dist_label[i,len(self.solution)] = np.argmin(dist_label[i,:len(self.solution)]) #Set the best matching centroid as label

        return np.mean([distance.euclidean(data,self.solution[int(dist_label[i,len(self.solution)])]) for data,i in zip(X,range(len(X)))])
...
def quantization_error(solution):
    ...
    for i in range(len(X)):
        ...
        dist_label[i, len(solution)] = np.argmin(dist_label[i, :len(solution)])

    return np.sum(np.square(np.array([distance.euclidean(data,solution[int(dist_label[i,len(solution)])]) for data,i in zip(X,range(len(X)))])))
...
print("PSO Quantization error Aritificial Dataset 1 = " + str(quant_error))
